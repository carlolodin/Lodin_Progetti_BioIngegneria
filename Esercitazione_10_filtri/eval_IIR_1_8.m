...
for k = 1:length(ordini)
    ...
    butter_filters{k} = designfilt("bandpassiir", FilterOrder= ordini(k), HalfPowerFrequency1=.9, ...
        HalfPowerFrequency2=8.1, SampleRate=sampling_rate, DesignMethod='butter');   
    toc;
end
...
xlim([0 10]);   % Zoom sul range di interesse
ylim([-50 5]); % Zoom sull'asse y
plot([1 1], ylim, 'k--', 'LineWidth', 1.2, 'DisplayName', '1 Hz');  %Plot linee guida della banda
...
xlim([-10,10]);
plot([1 1], ylim, 'r--', 'LineWidth', .8, 'DisplayName', '1 Hz'); %Linee guida per la banda
...
for k = 1:length(ordini)
    ...
    cheby1_filters{k} = designfilt("bandpassiir", FilterOrder= ordini(k), PassbandFrequency1=1, ...
        PassbandFrequency2=8, SampleRate=sampling_rate, DesignMethod='cheby1', PassBandRipple=0.1);   
    toc;
end
...
for k = 1:length(ordini)
    ...
    cheby2_filters{k} = designfilt("bandpassiir", FilterOrder= ordini(k), StopbandFrequency1=.9, ...
        StopbandFrequency2=8.1, SampleRate=sampling_rate, DesignMethod='cheby2', StopBandAttenuation=6);   
    toc;
end
...
for k = 1:length(ordini)
    ...
    elliptic_filters{k} = designfilt("bandpassiir", FilterOrder= ordini(k), PassbandFrequency1=1, ...
        PassbandFrequency2=8, SampleRate=sampling_rate, DesignMethod='ellip', StopbandAttenuation1=20, PassbandRipple=0.1, ...
...
plot(t, filter(butter_filters{4}, data), 'DisplayName', 'IIR Butter Order 60'); % Filtro FIR con metodo della finestra
plot(t, filter(cheby1_filters{4}, data), 'DisplayName', 'IIR Cheby1 Order 26'); % Filtro FIR con metodo equiripple
plot(t, filter(cheby2_filters{4}, data), 'DisplayName', 'IIR Cheby2 Order 60'); % Filtro FIR con metodo least-squares
